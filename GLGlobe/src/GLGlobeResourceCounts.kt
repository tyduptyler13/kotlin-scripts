 ...
 * @author Tyler Scott
 */
class GLGlobeResourceCounts : CliktCommand(
    ...
    private val databaseLocation: File by argument(
        name = "GeoLite2_Database",
        help = "The GeoLite2 mmdb file"
    ).file(
        exists = true,
        fileOkay = true,
        readable = true
    ...
    override fun run() {
        val globeOutput = GLGlobeOutput(
            DatabaseReader.Builder(databaseLocation)
                .withCache(CHMCache())
                .build(),
            whitelist?.readLines()?.toHashSet() ?: emptySet()
        )

        readGoogleAccessLogs(googleAccessLogDirectory) { log, _ ->
            globeOutput.collectSome(log)
        }
...
fun main(args: Array<String>) = GLGlobeResourceCounts().main(args)
